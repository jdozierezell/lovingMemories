...
const Notifications = props => {
    ...
    const handleSuccess = (data) => {
        ...
        let details = {}
        handleAPIPost('user/notifications/all/read', details, state.data.auth_token, handleNotiAllReadSuccess, handleNotiAllReadErrors);
    ...
    const checkItem = (token, id) => {
        ...
        if (token.includes("/view-memory")) {
            ...
            handleAPIPost('memory/notifications/all/read', itemID, state.data.auth_token, handleReadSuccess, handleReadErrors);
            navigate(token);
    ...
    useEffect(() => {
        ...
        handleAPIPost('memory/notifications', values, data.auth_token, handleSuccess, handleErrors);
                            ...
                            renderItem={item => (
                                <List.Item className="relative">
                                    <List.Item.Meta
                                        ...
                                        title={<p dangerouslySetInnerHTML={{__html: item.text}}/>}
                                        description={item.ago}
                                    />                                  
                                    {(item.read === 0) ? (
                                        <span
                                            className="your-memory-notifications block bg-pink-500 w-4 h-4 md:w-4 md:h-4 absolute left-24 md:left-20 top-10 rounded-full"></span>
                                    ) : (
                                        <></>
                                    )}
                                    {(item.disable_button === false) ? (
                                        <div className="pr-2 md:pr-0">
                                            <button onClick={() => checkItem(item.url, item.id)}
