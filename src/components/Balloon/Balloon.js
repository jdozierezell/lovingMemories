...
const Balloon = () => {
    ...
    const createBalloons = (memObj) => {
        ...
        for (let i = 0; i < memObj.length; i++) {
            ...
            if (memObj[i].name.length > 18) {
                balloon.setAttribute('name', memObj[i].name.substring(0, 15).toLowerCase() + '...');
            }
            else {
    ...
    const fetchMoreData = () => {
        if (fetchCount <= 1) {
            ...
            handleAPIPost('memory/latest', details, "", handleNextSuccess, handleNextErrors);
        }
    ...
    function onScroll(event) {
        ...
        if (window.innerWidth < 900) {
            if (element.scrollWidth - element.scrollLeft <= (element.clientWidth + element.clientWidth/3))
            {
                if(fetchCount <= 1) {
            ...
            if (element.scrollHeight - element.scrollTop <= Math.round(element.clientHeight + element.clientHeight/5))
            {
            ...
            <div id="balloonHolder" className="balloon-holder">
                <div id="balloon-container">
                    ...
                    <div className='absolute bottom-0 left-0 flex items-center justify-end pr-10 w-full z-20 h-20 bg-opacity-0'>
                        <LoadingOutlined style={{ fontSize: 40, color:"#3369ff" }} spin />
                    </div>
                    }
