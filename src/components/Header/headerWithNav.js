...
function Main({ siteTitle, isLanding }) {
  ...
  useEffect(() => {
    ...
    if (getUserDetails.user === undefined) {
      ...
      handleAPIPost(
        "user/check/new/notifications",
        details,
        getUserDetails.token,
        handleNotificationReadSuccess,
        handleNotificationReadErrors
      )
    }
  }, [])
  ...
  return (
    <div>
      <div className="fixed top-0 left-0 lg:absolute w-full z-40 bg-white bg-opacity-20 lg:bg-transparent  lg:shadow-none header-nav">
        {isLanding ? (
          <></>
        ) : (
          <div
            className={`flex items-center justify-between py-0 px-0 ${
              isUserLoggedIn ? "bg-white bg-opacity-20" : ""
            }`}
          >
            <div>
              <Link to="/">
                <span className="absolute bg-white">
                  <img
                    src="../../svg/afsp-logo-lovingmemories.svg"
                    className="cursor-pointer h-20 lg:h-24 max-w-xs w-11/12 "
                    placeholder="white"
                    alt="logo"
                  />
                </span>
            ...
            {isUserLoggedIn ? (
              <div className="relative w-full top-0 pointer-events-none hidden lg:block">
                <div className="flex gap-10 items-center justify-center mx-auto pointer-events-auto header-nav">
                  <div className="text-lg font-bold ">
                    <Link to="/app/your-memories">
                      <span className="text-gray-400 text-sm">Memories</span>
                    </Link>
                  </div>
                  <div className="text-lg font-bold ">
                    <Link to="/app/notifications">
            ...
            >
              {isUserLoggedIn ? (
                <Dropdown overlay={menu} trigger={["click"]}>
                  <a
                    className="ant-dropdown-link menu-btn absolute right-4 top-4"
                ...
                <Link to="/app/login">
                  <span>
                    <button className="btn-signin inline-block sm:px-8 w-20 md:w-auto text-xs md:text-lg leading-none bg-white text-blue-500 border border-blue-500 rounded-full hover:border-transparent hover:text-white hover:bg-blue-500 lg:mt-0 absolute right-4 top-4">
                      <span>Sign In</span>
                    </button>
                  </span>
                </Link>
      ...
      {isLanding ? (
        ...
        <div>
          {isUserLoggedIn ? (
            <div
              className={`fixed w-full bottom-0 left-0 flex items-center justify-center px-0 py-0 mobile-nav-bar lg:hidden ${
                isUserLoggedIn ? "bg-white bg-opacity-20" : ""
              }`}
            >
