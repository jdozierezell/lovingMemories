...
import {handleAPIPost, isLoggedIn, getCurrentUser, logout} from "../../utils/auth"
...
const HeaderPlain = ({title, goBack, gobackUrl}) => {
    ...
    const menu = (
        <Menu>
            <Menu.Item style={{padding: "10px 30px 0 30px"}} key="0">
                <span className="pointer-events-none text-xs text-gray-400">Loggin in as:</span>
    ...
    return (
        <div className="lg:mb-10">
            ...
            <div className={`flex items-center justify-between p-4 ${isUserLoggedIn ? "bg-white " : ""}`}>
                <div  className="flex items-center">
                    {canGoBack ? (
                        <span>
              <Link to={newLink}>
                <div
                    className="border-2 border-black rounded-full cursor-pointer bg-white p-1 text-black flex item-center justify-center mr-5">
                ...
                <div  className="flex items-center justify-end">
                    {isUserLoggedIn ? (
                        ...
                        <Link to="/app/login">
                            <button
                                className="btn-signin inline-block px-4 py-1 w-20 md:w-auto text-xs md:text-lg leading-none bg-white text-blue-500 border border-blue-500 rounded-full hover:border-transparent hover:text-white hover:bg-blue-500 lg:mt-0">
                                Sign In
                            </button>
                        </Link>
                    )}
